...
const p1 = princesses.map(function(p) {
  return p.name;
});
...
const p2 = princesses.filter(function(p) {
...
const total = princesses.reduce(function(acc, cur) {
  return acc += cur.age
}, 0)
...
const nickNames = princesses.filter(function(p) {
  return p.name.includes("l") || p.name.includes("L");
});
